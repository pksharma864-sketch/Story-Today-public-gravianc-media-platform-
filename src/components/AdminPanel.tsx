...
export const AdminPanel: React.FC<Props> = ({
  ...
  const [userFilter, setUserFilter] = useState<'all' | 'pending' | 'active' | 'suspended_or_rejected'>('all');
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setUserActionMsg({ type: 'success', text: `User account "${newUserUsername}" created and activated successfully!` });
        setNewUserName('');
        setNewUserUsername('');
        setNewUserPassword('');
        setNewUserEmail('');
  ...
  const handleApproveUser = async (userId: string, userName: string) => {
    ...
    try {
      const res = await approveUser(userId, 'Chief Admin');
      if (res.success && res.user) {
        setUsersList((prev) => prev.map((u) => (u.id === userId ? res.user! : u)));
        setUserActionMsg({
          type: 'success',
          text: `Account for "${userName}" (@${res.user.username}) approved and activated successfully!`,
        });
        setTimeout(() => setUserActionMsg(null), 4000);
      } else {
        setUserActionMsg({ type: 'error', text: res.error || 'Failed to approve user' });
      }
    } catch (err) {
      setUserActionMsg({ type: 'error', text: 'Network error while approving user' });
    } finally {
  ...
  const handleRejectUser = async (userId: string) => {
    ...
    setUserActionMsg(null);
    try {
      const reason = userRejectReason.trim() || 'Registration application declined by Administrator';
      ...
      if (res.success && res.user) {
        ...
        setUserActionMsg({ type: 'error', text: res.error || 'Failed to reject user' });
      }
    } catch (err) {
      setUserActionMsg({ type: 'error', text: 'Network error while rejecting user' });
    } finally {
      setUserActionLoadingId(null);
    }
  };
  ...
  const handleSuspendUser = async (userId: string, userName: string) => {
    if (!confirm(`Are you sure you want to suspend account for "${userName}"? They will not be able to log in.`)) return;
    setUserActionLoadingId(userId);
    setUserActionMsg(null);
    try {
      const res = await updateUserStatus(userId, 'suspended');
      if (res.success && res.user) {
        setUsersList((prev) => prev.map((u) => (u.id === userId ? res.user! : u)));
        setUserActionMsg({
          type: 'success',
          text: `Account for "${userName}" has been suspended.`,
        });
        setTimeout(() => setUserActionMsg(null), 4000);
      } else {
        setUserActionMsg({ type: 'error', text: res.error || 'Failed to suspend user' });
      }
    } catch (err) {
      setUserActionMsg({ type: 'error', text: 'Network error while suspending user' });
    } finally {
      setUserActionLoadingId(null);
    }
  };
  ...
  const handleActivateUser = async (userId: string, userName: string) => {
    ...
    setUserActionMsg(null);
    try {
      const res = await updateUserStatus(userId, 'active', { approvedBy: 'Chief Admin' });
      if (res.success && res.user) {
        setUsersList((prev) => prev.map((u) => (u.id === userId ? res.user! : u)));
        setUserActionMsg({
          type: 'success',
          text: `Account for "${userName}" re-activated successfully!`,
        });
        setTimeout(() => setUserActionMsg(null), 4000);
      } else {
        setUserActionMsg({ type: 'error', text: res.error || 'Failed to activate user' });
      }
    } catch (err) {
      setUserActionMsg({ type: 'error', text: 'Network error while activating user' });
    } finally {
      setUserActionLoadingId(null);
    }
  };
  ...
  const handleDeleteUser = async (userId: string, userName: string) => {
    if (!confirm(`Are you sure you want to permanently remove user account "${userName}"?`)) return;
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-1.5 border-b border-[#E0E0E0] pb-2 overflow-x-auto no-scrollbar">
                ...
                >
                  ...
                  {usersList.filter((u) => u.status === 'pending').length > 0 && (
                    <span className="px-1.5 py-0.5 rounded-full text-[10px] font-mono bg-amber-500 text-white font-bold animate-pulse">
              ...
              {adminTab === 'users' && (
                <div className="space-y-4">
                  ...
                  {/* User Accounts Filter Tabs & Search */}
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-1">
                    <div className="flex items-center gap-1.5 p-1 bg-gray-100 rounded-lg text-xs font-bold uppercase tracking-wider overflow-x-auto no-scrollbar">
                      <button
                        type="button"
                      ...
                      <button
                        type="button"
                        onClick={() => setUserFilter('suspended_or_rejected')}
                        className={`px-3 py-1.5 rounded-md transition-all whitespace-nowrap flex items-center gap-1.5 ${
                          userFilter === 'suspended_or_rejected'
                      ...
                      >
                        ...
                        <span className="px-1.5 py-0.2 rounded-full text-[10px] font-mono bg-gray-200 text-gray-700">
                          {usersList.filter((u) => u.status === 'suspended' || u.status === 'rejected').length}
                        </span>
                      </button>
                    </div>
                    ...
                    <div className="relative min-w-[200px]">
                      <input
                        ...
                        className="w-full text-xs pl-3 pr-8 py-1.5 rounded-lg border border-gray-300 bg-white focus:outline-hidden focus:border-[#004D40]"
                      />
                      {userSearch && (
                        <button
                          type="button"
                  ...
                  {/* Pending Alert Banner if pending registrations exist */}
                  {usersList.some((u) => u.status === 'pending') && userFilter !== 'pending' && (
                    <div className="p-3 bg-amber-50 border border-amber-300 rounded-xl flex items-center justify-between gap-3 text-xs text-amber-900">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          <strong>{usersList.filter((u) => u.status === 'pending').length} new account registration(s)</strong> are waiting for your approval.
                        </span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setUserFilter('pending')}
                        className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded-md font-bold text-[11px] uppercase tracking-wider shrink-0 transition-colors shadow-2xs"
                      >
                        Review Pending
                      </button>
                    </div>
                        ...
                        <tbody className="divide-y divide-[#E0E0E0]">
                          {usersList
                            .filter((u) => {
                              ...
                              if (userFilter === 'active') return u.status === 'active' || !u.status;
                              if (userFilter === 'suspended_or_rejected') return u.status === 'suspended' || u.status === 'rejected';
                              return true;
                            })
                            .filter((u) => {
                                  ...
                                  <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                      <div className="font-bold text-gray-900 flex items-center gap-1.5">
                                        <span>{u.name}</span>
                                        {u.idCard && (
                                          <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-teal-100 text-teal-900 border border-teal-300" title="Has Press ID application">
                                            ID Card
                                          </span>
                                        )}
                                      </div>
                                ...
                                <td className="p-3 text-right">
                                  <div className="flex items-center justify-end gap-1.5">
                                    ...
                                    {u.username === 'admin' || u.id === 'user_admin' ? (
                                      <span className="text-[10px] text-gray-400 font-mono font-bold px-2 py-1 bg-gray-100 rounded">
                                        Super Admin
                                      </span>
                                    ) : (
                                      <>
                                        ...
                                        {u.status === 'pending' && (
                                          <>
                                            <button
                                              ...
                                              className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-[11px] font-bold uppercase tracking-wider flex items-center gap-1 shadow-2xs transition-colors cursor-pointer"
                                            ...
                                            <button
                                              type="button"
                                              onClick={() => setSelectedUserForReject(u.id)}
                                              disabled={userActionLoadingId === u.id}
                                              className="px-2 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded text-[11px] font-bold uppercase tracking-wider flex items-center gap-1 transition-colors cursor-pointer"
                                        ...
                                        {u.status === 'active' && (
                                          <button
                                            ...
                                            disabled={userActionLoadingId === u.id}
                                            className="px-2 py-1 text-gray-500 hover:text-amber-700 hover:bg-amber-50 rounded text-[11px] font-medium transition-colors"
                                        ...
                                        {(u.status === 'suspended' || u.status === 'rejected') && (
                                          <button
                                            ...
                                            disabled={userActionLoadingId === u.id}
                                            className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 rounded text-[11px] font-bold uppercase tracking-wider flex items-center gap-1 transition-colors"
                                            title="Re-activate and approve account"
                                          >
                                            <Check className="w-3 h-3" />
                                            <span>Activate</span>
                                          </button>
                  ...
                  {selectedUserForReject && (
                    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 p-4">
                      <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-4 space-y-3 border border-gray-300">
                        <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-gray-600">
                          Please provide an optional reason for declining this registration request:
                        </p>
                        <textarea
                          rows={3}
                          value={userRejectReason}
                          onChange={(e) => setUserRejectReason(e.target.value)}
                          placeholder="e.g. Identity verification could not be completed, or invalid credentials."
                          className="w-full text-xs p-2.5 rounded-lg border border-gray-300 bg-white focus:outline-hidden focus:border-rose-500"
                        />
                        <div className="flex justify-end gap-2 pt-1">
                          ...
                          <button
                            type="button"
                            onClick={() => handleRejectUser(selectedUserForReject)}
                            disabled={userActionLoadingId === selectedUserForReject}
                            className="px-4 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-md text-xs font-bold uppercase tracking-wider shadow-xs"
                          >
                            Confirm Rejection
                          </button>
                        </div>
